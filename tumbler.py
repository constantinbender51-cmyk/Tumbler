...
# III Parameters
III_WINDOW = 14  # 14-day window for III calculation
III_T_LOW = 0.10  # Below this: 0x leverage (too choppy)
III_T_HIGH = 0.50  # Above this: 1.5x leverage (overextended)
LEV_LOW = 0.0   # Choppy market
LEV_MID = 3.0   # Trending market
LEV_HIGH = 1.5  # Overextended market
...
def calculate_iii(df: pd.DataFrame) -> float:
    ...
    # Get last 14 days + 1 for calculating returns
...
def determine_leverage(iii: float) -> float:
    ...
    - III < 0.10: 0x (too choppy, stay out)
    - 0.10 <= III < 0.50: 3x (trending nicely)
    - III >= 0.50: 1.5x (overextended, reduce risk)
    """
    if iii < III_T_LOW:
        return LEV_LOW  # 0x - no trade
    elif iii < III_T_HIGH:
        return LEV_MID  # 3x - full risk
    else:
        return LEV_HIGH  # 1.5x - reduced risk
...
def place_take_profit(api: kf.KrakenFuturesApi, side: str, size_btc: float, fill_price: float):
    ...
    if side == "buy":
        # LONG position: TP is sell limit 16% above entry
        tp_price = fill_price * (1 + TAKE_PROFIT_PCT)
        tp_side = "sell"
    else:
        # SHORT position: TP is buy limit 16% below entry
        tp_price = fill_price * (1 - TAKE_PROFIT_PCT)
        tp_side = "buy"
...
def smoke_test(api: kf.KrakenFuturesApi):
    ...
    try:
        ...
        test_collateral = usd
        test_notional = test_collateral * leverage
        ...
        # Test take profit order format
        log.info(f"Test TAKE PROFIT order (not placing, just checking format):")
        tp_price_sell = mp * (1 + TAKE_PROFIT_PCT)
        
        log.info(f"  For a LONG position (sell TP):")
        log.info(f"    Entry price: ${mp:.2f}")
        log.info(f"    TP price: ${tp_price_sell:.2f} (+{TAKE_PROFIT_PCT * 100}%)")
        log.info(f"    TP price (int): {int(round(tp_price_sell))}")
...
def daily_trade(api: kf.KrakenFuturesApi):
    ...
    # === CALCULATE III AND DETERMINE LEVERAGE ===
    iii = calculate_iii(df)
    leverage = determine_leverage(iii)
    ...
    if leverage == 0:
        log.info("Leverage is 0x - market too choppy, forcing FLAT signal")
    ...
    # Override signal to FLAT if leverage is 0
    if leverage == 0:
        signal = "FLAT"
        log.info("Signal overridden to FLAT due to 0x leverage (choppy market)")
